...
export function CriarUsuario() {
  return (
    <div className="card-surface rounded-xl p-4 space-y-3">
      ...
      <div className="text-sm text-muted-foreground">Formulário de criação (mock). Em breve integraremos ao backend.</div>
      <div className="grid sm:grid-cols-2 gap-3 text-sm">
        <input className="rounded-md bg-background border px-3 py-2" placeholder="Nome" />
        <input className="rounded-md bg-background border px-3 py-2" placeholder="E-mail" />
        ...
        <button className="rounded-md bg-primary text-primary-foreground px-4 py-2 w-fit">Salvar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="card-surface rounded-xl p-4">
      ...
      <ul className="text-sm space-y-2">
        {bloqueados.length ? bloqueados.map(u => <li key={u.id}>{u.nome} — {u.email}</li>) : <li className="text-muted-foreground">Nenhum bloqueio.</li>}
      </ul>
    </div>
  );
}
  ...
  return (
    <div className="card-surface rounded-xl p-4 text-sm">
      ...
      <ul className="space-y-2">
        {agentes.map(a => <li key={a.id}>{a.nome} — {a.email}</li>)}
      </ul>
    </div>
  );
}
