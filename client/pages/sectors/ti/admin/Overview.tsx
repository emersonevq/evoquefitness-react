...
const daily = Array.from({ length: 7 }).map((_, i) => ({ day: ["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"][i], abertos: Math.floor(Math.random()*10)+2 }));
const weekly = Array.from({ length: 4 }).map((_, i) => ({ semana: `S${i+1}`, chamados: Math.floor(Math.random()*40)+10 }));
const pieData = [
        ...
        <div className="card-surface rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={daily}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="abertos" stroke="hsl(var(--primary))" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div className="card-surface rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-surface rounded-xl p-4">
          ...
          <ul className="text-sm text-muted-foreground space-y-2">
            <li>Tempo médio de resolução: <span className="text-foreground font-medium">6h 12m</span></li>
            <li>Primeira resposta: <span className="text-foreground font-medium">28m</span></li>
            <li>Reaberturas: <span className="text-foreground font-medium">3%</span></li>
            <li>Backlog: <span className="text-foreground font-medium">14</span></li>
          </ul>
        </div>
      </div>
    </div>
